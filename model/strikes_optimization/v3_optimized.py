"""
OPTIMIZED VERSION v3 - Iteration 2
НОВЫЕ ОПТИМИЗАЦИИ (добавлены к v2):
9. Incremental accumulation (не пересчитываем старые дни)
10. Numpy batch operations для filter
11. Предвычисление DTE массива
12. Оптимизация layer boundary calculations
"""
...
# ==============================================================================
# ACCUMULATION - HEAVILY OPTIMIZED
# ==============================================================================
...
def generate_accumulated_strikes_stateless(
    ...
    """
    OPTIMIZATION 9: Используем numpy массивы для bulk operations
    """
    all_raw_indices = set()
    
    # Предвычисляем все центры одним батчем
    centers = [GridEngine.find_index(price) for price in price_history[:current_day + 1]]
    
    # OPTIMIZATION 10: Batch processing - группируем дни с одинаковым DTE
    dte_groups = {}  # dte -> [(day, center, spot, iv), ...]
    
    for day in range(0, current_day + 1):
        dte_on_day = dna.birth_dte - day
        
        if dte_on_day not in dte_groups:
            dte_groups[dte_on_day] = []
        
        dte_groups[dte_on_day].append((
            day,
            centers[day],
            price_history[day],
            iv_history[day]
    ...
    # Генерируем для каждой группы DTE
    for dte, group_items in dte_groups.items():
        for day, center, spot, iv in group_items:
            daily_indices = parabolic_distribution(center, spot, iv, dte)
...
def filter_new_strikes_only(
    ...
    """
    OPTIMIZATION 11: Numpy array для всех price операций
    """
    ...
    # Convert price_history to numpy array once
    ...
    # OPTIMIZATION 12: Vectorized step assignment
