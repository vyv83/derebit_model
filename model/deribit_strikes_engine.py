...
class AlgoConfig:
    """
    Центральный класс конфигурации всей системы.
    Все параметры собраны здесь для удобства калибровки.
    """

    # ==========================================================================
    # 1. НАСТРОЙКИ СЕТКИ (GRID FOUNDATION)
    # Определяют фундамент: насколько густыми могут быть страйки в принципе.
    # ==========================================================================
    
    # ЧТОБЫ изменить плотность самой мелкой сетки (для BTC < 20k или дешевых активов):
    # Уменьшите (например до 0.01), чтобы шаги были мельче. 
    # Увеличьте (например до 0.05), чтобы сделать сетку грубее и реже.
    GRID_STEP_LOW_MULTIPLIER: float = 0.025 

    # ЧТОБЫ изменить плотность для основных цен (BTC > 50k):
    # Стандартно 5% от порядка цены (шаг 5000 при цене 100k).
    # Уменьшите (до 0.025), чтобы шаги стали чаще (шаг 2500 при 100k).
    GRID_STEP_HIGH_MULTIPLIER: float = 0.050

    # ЧТОБЫ сместить границу переключения на "среднюю" сетку:
    # Увеличьте (до 3.0), чтобы "мелкая" сетка работала до более высоких цен.
    GRID_THRESHOLD_LOW: float = 2.0

    # ЧТОБЫ сместить границу переключения на "крупную" сетку:
    # Увеличьте (до 8.0), если хотите затянуть переход на самый крупный шаг.
    GRID_THRESHOLD_HIGH: float = 5.0

    # ==========================================================================
    # 2. НАСТРОЙКИ ПАРАБОЛЫ (ГЕНЕРАЦИЯ БЕЗ ЛИМИТОВ)
    # Определяют, сколько "сырых" страйков мы генерируем каждый день.
    # ==========================================================================
    
    # --- НАСТРОЙКИ РАЗМАХА (ШИРИНЫ ДОСКИ) ---

    # ЧТОБЫ изменить общий охват страйков (сигма-коридор):
    # Увеличьте (до 2.0+), чтобы видеть ОЧЕНЬ далекие страйки (глубокий OTM).
    # Уменьшите (до 1.0), чтобы оставить только область активных торгов.
    PARABOLA_SIGMA_MULTIPLIER: float = 1.4

    # 🔥 ЧТОБЫ настроить, как сильно ВРЕМЯ (DTE) влияет на ШИРИНУ доски:
    # Уменьшите (до 0.1), чтобы дальние экспирации (LEAPS) были УЖЕ.
    # Увеличьте (до 0.4), чтобы дальние доски были огромными по охвату цен.
    PARABOLA_SIGMA_TIME_POWER: float = 0.2

    # 🔥 ЧТОБЫ настроить, как сильно ВОЛАТИЛЬНОСТЬ (IV) влияет на РАЗМАХ:
    # Увеличьте (до 3.0+), чтобы при росте IV доска сужалась агрессивнее.
    # Уменьшите (до 0.5), чтобы ширина доски была стабильнее и меньше зависела от волатильности.
    PARABOLA_IV_POWER: float = 2.0

    # --- НАСТРОЙКИ ПЛОТНОСТЬ (ГУСТОТЫ ВНУТРИ) ---

    # ЧТОБЫ изменить влияние ВРЕМЕНИ на ПЛОТНОСТЬ в центре (ATM):
    # Увеличьте (до 2.0), чтобы дальние экспирации стали заметно РЕДЖЕ в центре.
    # Уменьшите (до 0.5), чтобы сохранить высокую плотность даже на LEAPS.
    PARABOLA_DTE_DENSITY_MULTIPLIER: float = 1.0 

    # ЧТОБЫ изменить скорость разрежения страйков к краям (крылья):
    # Увеличьте (до 30.0), чтобы страйки на краях становились очень редкими мгновенно.
    # Уменьшите (до 5.0), чтобы плотность была более равномерной по всей доске.
    PARABOLA_STEEPNESS: float = 15.0  

    # ЧТОБЫ изменить форму кривой разрежения:
    # Увеличьте (до 2.0), чтобы сделать центр плотным, а переходы к крыльям - резкими.
    # Установите 1.0 для линейного (плавного) разрежения.
    PARABOLA_POWER: float = 1.2

    # ==========================================================================
    # 3. НАСТРОЙКИ МАГНИТА (ШАГ ОТ ВРЕМЕНИ)
    # Финальная фильтрация: жесткое прореживание в зависимости от DTE.
    # ==========================================================================
    
    # ЧТОБЫ изменить порог "долгих" контрактов:
    # Уменьшите (до 90), чтобы страйки начинали прореживаться раньше.
    MAGNET_THRESHOLD_LONG: int = 180

    # ЧТОБЫ изменить порог "средних" контрактов:
    # Уменьшите (до 14), чтобы плотная "дневная" сетка работала только в самом конце.
    MAGNET_THRESHOLD_MID: int = 30

    # ЧТОБЫ изменить шаг для дальних LEAPS:
    # Увеличьте (до 8), чтобы оставить только самые "круглые" страйки на дальних датах.
    MAGNET_STEP_LONG: int = 4

    # ЧТОБЫ изменить шаг для средних контрактов:
    # Установите 1, если хотите максимальную плотность для всего, что старше месяца.
    MAGNET_STEP_MID: int = 2

    # ЧТОБЫ изменить шаг для коротких контрактов:
    # 1 — используются все возможные страйки сетки (максимальная точность).
    MAGNET_STEP_SHORT: int = 1

    # ==========================================================================
    # 4. НАСТРОЙКИ ФИЛЬТРАЦИИ ИСТОРИИ (СЛОИ)
    # Как история движения цены влияет на сохранение страйков.
    # ==========================================================================
    
    # ЧТОБЫ изменить окно "горячей" истории (Слой 1):
    # Увеличьте (до 60), чтобы плотная сетка сохранялась дольше после ценового скачка.
    LAYER_WINDOW_RECENT: int = 30

    # ЧТОБЫ изменить окно "затухающей" истории (Слой 2):
    # Увеличьте (до 365), чтобы помнить старые уровни цен целый год.
    LAYER_WINDOW_MEDIUM: int = 180

    # ЧТОБЫ расширить зону покрытия вокруг свежих цен:
    # Увеличьте (до 0.10, т.е. 10%), чтобы дать больше пространства для маневра цене.
    LAYER1_BUFFER_PCT: float = 0.05

    # ЧТОБЫ расширить зону покрытия для старых уровней:
    LAYER2_BUFFER_PCT: float = 0.15

    # ЧТОБЫ изменить шаг прореживания для средних цен:
    LAYER2_MIN_STEP: int = 2

    # ЧТОБЫ изменить шаг для очень старых цен:
    LAYER3_MIN_STEP: int = 4

    # ==========================================================================
    # 5. НАСТРОЙКИ ГЛУБИНЫ ИСТОРИИ (LOOKBACKS)
    # Используются для визуализации слоев в приложении (Historical Ranges).
    # ==========================================================================

    # ЧТОБЫ изменить визуальное окно "Дневного" слоя:
    DAILY_LOOKBACK: int = 7

    # ЧТОБЫ изменить визуальное окно "Недельного" слоя:
    WEEKLY_LOOKBACK: int = 30

    # ЧТОБЫ изменить визуальное окно "Месячного" слоя:
